...
class SalesDailyReportInsightService
{
    public function getInsight(array $filters = []): array
    {
        [$dateFrom, $dateTo] = $this->resolvePeriod($filters);

        $current = $this->getSummary($dateFrom, $dateTo);
        ...
        $previous = $this->getSummary($previousFrom, $previousTo);

        $rates = $this->buildRates($current);
        $previousRates = $this->buildRates($previous);
    ...
    public function buildRates(array $summary): array
    {
